module Spree
  module Admin
    module PaperTrailManager
      class ChangesController < Spree::Admin::BaseController
        ...
        def update
          ...
          if @result
            if @version.event == "create"
              ...
              redirect_to spree.admin_change_url(@version)
            end
          else
      ...
      protected
        ...
        def change_index_allowed?          
          return SpreePaperTrailManager::Engine.allow_index?(self)
        end
        helper_method :change_index_allowed?

        # Allow show?
        def change_show_allowed?(version)
          return SpreePaperTrailManager::Engine.allow_show?(self, version)
        end
        helper_method :change_show_allowed?

        # Allow revert?
        def change_revert_allowed?(version)
          return SpreePaperTrailManager::Engine.allow_revert?(self, version)
        end
